...
async def get_replay(
    ...
    events_result = await db.execute(
        ...
        .order_by(GameEvent.created_at)         # 按创建时间升序，还原游戏时间线
    )
...
def _event_description(event: GameEvent) -> str:
    ...
    用于回放界面展示，例如：
    - event_type="speech", data={"seat": 3} → "3 号玩家发言"
    - event_type="game_over", data={"winner": "villager"} → "游戏结束 - villager 阵营胜利"
    """
    import json
    ...
    desc_map = {
        ...
        "last_words": f"{data.get('seat', '?')} 号玩家发表遗言",
        "speech": f"{data.get('seat', '?')} 号玩家发言",
        "vote": f"{data.get('seat', '?')} 号玩家投票",
        "vote_result": "投票结果公布",
        "eliminate": f"{data.get('seat', '?')} 号玩家被淘汰",
        "victory_check": "胜负检查",

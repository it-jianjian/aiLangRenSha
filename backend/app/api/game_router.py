...
from app.models.game import Game, GamePlayer, GameMode, GameStatus, PlayerType
...
async def create_game(
    ...
    game = await service.create_game(request)
...
async def get_game(
    ...
    detail = GameDetail(
        ...
        end_reason=game.end_reason, players=players,
    )
...
async def _verify_human_player(game_id: str, player_seat: int, db: AsyncSession):
    """验证请求者是否为该对局的人类玩家
    ...
    if player_seat != human_player.seat_number:
        raise HTTPException(status_code=403, detail="座位号与人类玩家不匹配")
...
async def night_action(
    ...
    db: AsyncSession = Depends(get_db),
    player_seat: int = Header(..., alias="X-Player-Seat", description="人类玩家座位号"),
):
    """夜晚行动（人类玩家提交）
    ...
    """
    await _verify_human_player(game_id, player_seat, db)
    service = GameService(db)
    await service.submit_night_action(game_id, request)
...
async def submit_speech(
    ...
    await _verify_human_player(game_id, player_seat, db)
    service = GameService(db)
    await service.submit_speech(game_id, request)
    return ApiResponse(data={"accepted": True})
...
async def submit_vote(
    ...
    await _verify_human_player(game_id, player_seat, db)
    service = GameService(db)
    await service.submit_vote(game_id, request)
    return ApiResponse(data={"accepted": True})
